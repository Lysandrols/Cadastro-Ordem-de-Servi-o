...
class OrdemServicoApp:
    def __init__(self, root):
        self.root = root
        self.root.title(f"LFL- INFORMÁTICA - Versão {__version__}")
        ...
        self.estilo.configure("TLabel", font=("Arial", 9), background="#f0f0f0")
        self.estilo.configure("TButton", font=("Arial", 11), padding=10, background="#8ccbf5", foreground="black")
        self.estilo.configure("TEntry", font=("Arial", 11), padding=10)
        ...
        self.tipo_label = ttk.Label(self.frame_principal, text="Tipo:", background="#f0f0f0")
        self.tipo_label.grid(row=0, column=0, padx=10, pady=10, sticky="w")
        self.tipo_combobox = ttk.Combobox(self.frame_principal, values=["Orçamento", "Ordem de Serviço"])
        self.tipo_combobox.grid(row=0, column=1, padx=10, pady=10, sticky="ew")

        self.nome_label = ttk.Label(self.frame_principal, text="Nome do Cliente:", background="#f0f0f0")
        self.nome_label.grid(row=1, column=0, padx=10, pady=10, sticky="w")
        self.nome_entry = ttk.Entry(self.frame_principal)
        self.nome_entry.grid(row=1, column=1, padx=10, pady=10, sticky="ew")
        self.nome_entry.bind("<FocusOut>", self.formatar_entrada)

        self.servico_label = ttk.Label(self.frame_principal, text="Serviço Realizado:", background="#f0f0f0")
        self.servico_label.grid(row=2, column=0, padx=10, pady=10, sticky="w")
        self.servico_button = ttk.Button(self.frame_principal, text="Selecionar Serviços", command=self.selecionar_servicos)
        self.servico_button.grid(row=2, column=1, padx=10, pady=10, sticky="ew")

        self.data_label = ttk.Label(self.frame_principal, text="Data (ddmmaaaa):", background="#f0f0f0")
        self.data_label.grid(row=3, column=0, padx=10, pady=10, sticky="w")
        self.data_entry = ttk.Entry(self.frame_principal)
        self.data_entry.grid(row=3, column=1, padx=10, pady=10, sticky="ew")
        self.preencher_data_atual()

        self.valor_label = ttk.Label(self.frame_principal, text="Valor Total:", background="#f0f0f0")
        self.valor_label.grid(row=4, column=0, padx=10, pady=10, sticky="w")
        self.valor_entry = ttk.Entry(self.frame_principal, state='readonly')
        self.valor_entry.grid(row=4, column=1, padx=10, pady=10, sticky="ew")

        self.telefone_label = ttk.Label(self.frame_principal, text="Telefone (com DDD, sem +):", background="#f0f0f0")
        self.telefone_label.grid(row=5, column=0, padx=10, pady=10, sticky="w")
        self.telefone_entry = ttk.Entry(self.frame_principal)
        self.telefone_entry.grid(row=5, column=1, padx=10, pady=10, sticky="ew")

        self.gerar_button = ttk.Button(self.frame_principal, text="Gerar Ordem", command=self.gerar_ordem)
        self.gerar_button.grid(row=6, column=0, columnspan=2, pady=10)

        self.buscar_button = ttk.Button(self.frame_principal, text="Buscar Ordem/Orçamento", command=self.abrir_busca)
        self.buscar_button.grid(row=7, column=0, columnspan=2, pady=10)

        self.desenvolvido_label = ttk.Label(root, text="Desenvolvido por Lysandro Luiz", anchor='w', background="#f0f0f0")
        self.desenvolvido_label.pack(side="bottom", padx=20, pady=10, fill="x")
        ...
        if 'servicos' in self.orcamentos:
            self.servicos_selecionados = self.orcamentos['servicos']
    ...
    def selecionar_servicos(self):
        ...
        for servico in servicos:
            ...
            if any(servico_selecionado[0] == servico for servico_selecionado in self.servicos_selecionados):
                var.set(True)
                valor_selecionado = next(
                    int(servico_selecionado[1]) for servico_selecionado in self.servicos_selecionados if servico_selecionado[0] == servico,
                      None
                     )
    ...
    def abrir_busca(self):
        ...
        self.busca_window.geometry("600x500")
        ...
        self.apagar_button = ttk.Button(self.busca_window, text="Apagar", command=self.apagar_orcamento)
        self.apagar_button.pack(padx=10, pady=10)
    ...
    def selecionar_orcamento(self):
        ...
        for orcamento in self.orcamentos:
            if orcamento['numero'] == numero_ordem:
                ...
                self.servicos_selecionados = orcamento['servicos']

        self.busca_window.destroy()

    def apagar_orcamento(self):
        selected_index = self.resultados_listbox.curselection()
        if not selected_index:
            messagebox.showerror("Erro", "Selecione um orçamento para apagar.")
            return
        ...
        for orcamento in self.orcamentos:
            if orcamento['numero'] == numero_ordem:
                self.orcamentos.remove(orcamento)
                self.salvar_orcamentos()
                messagebox.showinfo("Sucesso", "Orçamento apagado com sucesso!")
                self.resultados_listbox.delete(selected_index)
                return
